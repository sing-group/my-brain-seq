...
cat "${deseq_file}" | tail -n +2 | awk '$7<5e-2' | sort > "${dint_output}/deseq_${vp_comparison_label}_qval-0.05.tsv"
...
cat "${edger_file}" | tail -n +2 | awk '$5<5e-2' | sort > "${dint_output}/edger_${vp_comparison_label}_qval-0.05.tsv"
...
cat <(cut -f1,3,7 "${dint_output}/deseq_${vp_comparison_label}_qval-0.05.tsv") \
	<(cut -f1,2,4 "${dint_output}/edger_${vp_comparison_label}_qval-0.05.tsv") > "${dint_output}/all_${vp_comparison_label}.tsv"
echo "[PIPELINE -- dea-integration]:	Saved in: ${dint_output}/all_${vp_comparison_label}.tsv"
...
cat "${dint_output}/edger_${vp_comparison_label}_qval-0.05.tsv" "${dint_output}/deseq_${vp_comparison_label}_qval-0.05.tsv" | cut -f 1 | sort | uniq -d > "${dint_output}/coincidences_${vp_comparison_label}.tsv"

#if coincidences.tsv is not empty, perform the integration
if [ -s "${dint_output}/coincidences_${vp_comparison_label}.tsv" ]; then
	...
	grep -f "${dint_output}/coincidences_${vp_comparison_label}.tsv" "${dint_output}/all_${vp_comparison_label}.tsv" > "${dint_output}/DEmiRNAs_${vp_comparison_label}_deseq-edger_integrated.tsv"
	sed -i '1s/^/Feature	log2FC	q-value\n/' "${dint_output}/DEmiRNAs_${vp_comparison_label}_deseq-edger_integrated.tsv"
	...
	docker run --rm \
		...
		pegi3s/r_data-analysis \
			Rscript ${workingDir}/compi_scripts/${deaIntRscript} ${dint_output_dk}/DEmiRNAs_${vp_comparison_label}_deseq-edger_integrated.tsv ${dint_output_dk} ${vp_comparison_label}
